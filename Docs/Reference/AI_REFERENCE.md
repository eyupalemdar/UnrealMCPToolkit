...
> 202 MCP tools across 182 native TCP commands in 38 categories plus 19 client-only tools. UE 5.8, TCP port auto-discovery plus multi-editor routing and native localhost HTTP/MCP probe support.
...
| `editor_call_many(commands, editor_id?, project_dir?, port?, scope?, dry_run?, stop_on_error?)` | Resolve one exact editor once, then execute up to 100 ordered commands with per-command scope/dry-run overrides; no cross-command rollback |
...
| `duplicate_asset(asset_path, target_asset_path?, new_package_path?, new_asset_name?, scope?, dry_run?)` | Duplicate one asset using AssetTools |
| `move_folder_assets(source_folder, target_folder, operation?, recursive?, scope?, dry_run?)` | Move or copy all assets under a folder using UE EditorAssetSubsystem directory APIs |
...
Folder move/copy details: `Docs/Usage/ASSET_FOLDER_OPERATIONS.md`.
...
the existing TCP protocol. Exact `editor_id` routing probes only the matching
registry record. Full editor listing and project-directory fallback probe
distinct ports concurrently so stale commandlet records do not add serial
timeout latency.
...
| `editor_call_many(commands, editor_id?, project_dir?, port?, scope?, dry_run?, stop_on_error?)` | Execute a bounded ordered command sequence after one exact target resolution; each command retains its own policy check and the batch is explicitly non-transactional |
...
editor_call_many(
    ...
    scope="read",
    commands=[
        {"command": "get_asset_properties", "params": {"asset_path": "/Game/UI/Hud/W_MainMenu"}},
        {"command": "get_dependencies", "params": {"asset_path": "/Game/UI/Hud/W_MainMenu"}},
    ]
)
