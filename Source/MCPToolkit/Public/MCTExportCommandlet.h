...
#include "MCTExportCommandlet.generated.h"

/**
 * Commandlet for exporting UE assets to text format for AI analysis.
 *
 * Uses the modular MCTExporterRegistry system — adding support for new asset types
 * only requires creating a new UMCTExporterBase subclass; no commandlet changes needed.
 *
 * Usage:
 *   UnrealEditor-Cmd.exe Project.uproject -run=MCTExport -asset="/Game/Path/To/Asset" [-simplify|-raw|-both] [-output="Dir"]
 *
 * Parameters:
 ...
 */
UCLASS()
class MCPTOOLKIT_API UMCTExportCommandlet : public UCommandlet
{
	GENERATED_BODY()

public:
	UMCTExportCommandlet();
...
private:
	...
	/** Export asset by path (delegates to UMCTExportFunctionLibrary) */
