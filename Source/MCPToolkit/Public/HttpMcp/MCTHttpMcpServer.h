...
{
struct FMCTHttpMcpToolDescriptor
{
	FString Name;
	FString Category;
...
struct FMCTHttpMcpCallbacks
{
	...
	TFunction<TArray<FMCTHttpMcpToolDescriptor>()> GetToolDescriptors;
	TFunction<bool()> IsStopRequested;
	TFunction<int32(int32, int32)> FindAvailablePort;
};

class FMCTHttpMcpServer
{
public:
	void Start(const FMCTHttpMcpCallbacks& InCallbacks);
	void Stop();

	FMCTHttpMcpStatus GetStatus() const;
...
private:
	...
	FMCTHttpMcpCallbacks Callbacks;
