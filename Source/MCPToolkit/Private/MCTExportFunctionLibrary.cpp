...
#define LOCTEXT_NAMESPACE "MCTExportFunctionLibrary"

//==========================================================================
// Asset Support Checks
//==========================================================================

bool UMCTExportFunctionLibrary::IsAssetTypeSupported(const FAssetData& AssetData)
{
...
bool UMCTExportFunctionLibrary::IsAssetTypeSupported(UObject* Asset)
{
	if (!Asset)
	{
		return false;
...
FString UMCTExportFunctionLibrary::ExportAssetContent(UObject* Asset, bool bFilterDefaults)
{
	if (!Asset)
	{
		return TEXT("");
	...
	if (!Registry)
	{
		UE_LOG(LogTemp, Error, TEXT("MCTExportFunctionLibrary: Failed to get exporter registry"));
		return TEXT("");
	}

	UMCTExporterBase* Exporter = Registry->FindExporterForAsset(Asset);
	if (!Exporter)
	{
		UE_LOG(LogTemp, Warning, TEXT("MCTExportFunctionLibrary: No exporter found for asset type %s"),
...
bool UMCTExportFunctionLibrary::ExportAsset(UObject* Asset, FMCTExportResult& OutResult)
{
	if (!Asset)
	{
	...
	UE_LOG(LogTemp, Log, TEXT("MCTExport: Starting export for asset '%s' (type: %s)"), *OutResult.AssetName, *OutResult.AssetType);
	...
	EMCTExportOutputMode OutputMode = Settings ? Settings->OutputMode : EMCTExportOutputMode::SimplifiedOnly;

	UE_LOG(LogTemp, Log, TEXT("MCTExport: Output directory: '%s'"), *OutputDir);

	// Handle output based on mode
	switch (OutputMode)
	{
		case EMCTExportOutputMode::RawOnly:
		{
		...
		case EMCTExportOutputMode::SimplifiedOnly:
		{
		...
		case EMCTExportOutputMode::Both:
		{
	...
	if (UTexture2D* Texture = Cast<UTexture2D>(Asset))
	{
		...
		if (Registry)
		{
			UMCTTextureExporter* TextureExporter = Cast<UMCTTextureExporter>(Registry->FindExporterForAsset(Asset));
			if (TextureExporter)
			{
				...
				{
					UE_LOG(LogTemp, Log, TEXT("MCTExport: Exported texture PNG to %s"), *PNGPath);
				}
				else
				{
					UE_LOG(LogTemp, Warning, TEXT("MCTExport: Failed to export texture PNG for %s"), *Asset->GetName());
				}
			}
		}
	}
...
int32 UMCTExportFunctionLibrary::ExportAssets(const TArray<FAssetData>& Assets, FOnMCTExportProgress OnProgress, TArray<FMCTExportResult>& OutResults)
{
...
FMCTExportResult UMCTExportFunctionLibrary::ExportAssetByPath(const FString& AssetPath, const FString& OutputDirectory, bool bBothFormats)
{
	FMCTExportResult Result;
	...
	if (bBothFormats)
	{
		...
		{
			FString StripScriptPath = FPaths::Combine(FMCTModule::GetScriptsDir(), TEXT("strip_utils.py"));
...
FMCTExportResult UMCTExportFunctionLibrary::ExportWidgetBlueprintByPath(const FString& AssetPath, const FString& OutputDirectory, bool bBothFormats)
{
	return ExportAssetByPath(AssetPath, OutputDirectory, bBothFormats);
}

FMCTExportResult UMCTExportFunctionLibrary::ExportBlueprintByPath(const FString& AssetPath, const FString& OutputDirectory, bool bBothFormats)
{
	return ExportAssetByPath(AssetPath, OutputDirectory, bBothFormats);
}
...
bool UMCTExportFunctionLibrary::RunSimplifier(const FString& RawFilePath, FString& OutSimplifiedPath)
{
...
void UMCTExportFunctionLibrary::CopyToClipboard(const FString& Content)
{
	FPlatformApplicationMisc::ClipboardCopy(*Content);
}

void UMCTExportFunctionLibrary::ShowExportNotification(int32 SuccessCount, int32 FailCount, const FString& OutputDirectory)
{
	FText Message;
...
void UMCTExportFunctionLibrary::OpenFileInEditor(const FString& FilePath)
{
	FPlatformProcess::LaunchFileInDefaultExternalApplication(*FilePath);
}

FString UMCTExportFunctionLibrary::GetOutputDirectory()
{
	const UMCTSettings* Settings = UMCTSettings::Get();
...
FString UMCTExportFunctionLibrary::GetOutputPathForAsset(UObject* Asset)
{
	if (!Asset)
	{
...
FString UMCTExportFunctionLibrary::GetSimplifierScriptPath()
{
	FString ScriptsDir = FMCTModule::GetScriptsDir();
...
bool UMCTExportFunctionLibrary::WriteToFile(const FString& Content, const FString& FilePath)
{
	// Ensure directory exists
	...
	UE_LOG(LogTemp, Log, TEXT("MCTExport: Writing %d bytes to '%s'"), Content.Len(), *AbsolutePath);

	bool bSuccess = FFileHelper::SaveStringToFile(Content, *FilePath, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM);

	if (bSuccess)
	{
		UE_LOG(LogTemp, Log, TEXT("MCTExport: Successfully wrote file"));
	}
	else
	{
		UE_LOG(LogTemp, Error, TEXT("MCTExport: FAILED to write file to '%s'"), *AbsolutePath);
	}

	return bSuccess;
}

FString UMCTExportFunctionLibrary::SanitizeFileName(const FString& InName)
{
...
FString UMCTExportFunctionLibrary::GetAssetTypeName(UObject* Asset)
{
	if (!Asset)
	{
		return TEXT("Unknown");
