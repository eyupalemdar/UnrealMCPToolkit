...
{
uint32 FMCTTcpTransport::Run(int32 ServerPort, const FMCTTcpTransportCallbacks& InCallbacks)
{
	UE_LOG(LogMCT, Log, TEXT("MCTExport TCP Server thread started on port %d"), ServerPort);

	ISocketSubsystem* SocketSubsystem = ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM);
	if (!SocketSubsystem)
	{
		UE_LOG(LogMCT, Error, TEXT("Failed to get socket subsystem"));
		return 1;
	}

	ListenerSocket = SocketSubsystem->CreateSocket(NAME_Stream, TEXT("MCTTcpServer"), false);
	if (!ListenerSocket)
	{
		UE_LOG(LogMCT, Error, TEXT("Failed to create listener socket"));
		return 1;
	}
	...
	{
		UE_LOG(LogMCT, Error, TEXT("Failed to bind to port %d"), ServerPort);
	...
	{
		UE_LOG(LogMCT, Error, TEXT("Failed to listen on socket"));
		SocketSubsystem->DestroySocket(ListenerSocket);
		ListenerSocket = nullptr;
		return 1;
	}

	UE_LOG(LogMCT, Log, TEXT("MCTExport TCP Server listening on 127.0.0.1:%d"), ServerPort);
	...
	while (!InCallbacks.IsStopRequested || !InCallbacks.IsStopRequested())
	{
		...
		if (ListenerSocket->WaitForPendingConnection(bHasPendingConnection, FTimespan::FromMilliseconds(100)))
		{
			if (bHasPendingConnection)
			{
				...
				FSocket* ClientSocket = ListenerSocket->Accept(*RemoteAddress, TEXT("MCTExportClient"));

				if (ClientSocket)
				{
					UE_LOG(LogMCT, Verbose, TEXT("Client connected"));
	...
	UE_LOG(LogMCT, Log, TEXT("MCTExport TCP Server stopped"));
	return 0;
}

void FMCTTcpTransport::Stop()
{
	if (ListenerSocket)
	{
...
void FMCTTcpTransport::HandleClientConnection(FSocket* ClientSocket, const FMCTTcpTransportCallbacks& InCallbacks)
{
	if (!ClientSocket)
	{
		return;
	...
	if (ClientSocket->Recv(RecvBuffer.GetData(), RecvBuffer.Num(), BytesRead))
	{
		if (BytesRead > 0)
		{
			...
			UE_LOG(LogMCT, Verbose, TEXT("Received command: %s"), *JsonCommand.Left(200));
			...
			UE_LOG(LogMCT, Verbose, TEXT("Sent response: %d bytes"), BytesSent);
		}
	}
	else
	{
		UE_LOG(LogMCT, Warning, TEXT("Failed to receive data from client"));
	}
}
}
