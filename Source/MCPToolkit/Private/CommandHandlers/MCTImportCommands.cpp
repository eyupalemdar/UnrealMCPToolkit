...
FString HandleImportTexture(TSharedPtr<FJsonObject> Params)
{
	...
	{
		FString Error;
		TSharedPtr<FJsonObject> Data = UMCTAssetImportBuilder::ImportTexture(
			SourcePath,
			PackagePath,
			AssetName,
			Compression,
...
FString HandleImportFont(TSharedPtr<FJsonObject> Params)
{
	...
	TArray<FMCTImportFontFaceSpec> Faces;
	...
	{
		FString Error;
		TSharedPtr<FJsonObject> Data = UMCTAssetImportBuilder::ImportFont(PackagePath, FontName, Faces, Hinting, Error);
		if (!Data.IsValid())
		{
