...
FString FMCTAsyncJobStore::CreateQueuedTask(const FString& CommandName)
{
	const FString TaskId = FGuid::NewGuid().ToString(EGuidFormats::DigitsWithHyphensLower);
	TSharedPtr<FMCTAsyncCommandJob> Job = MakeShared<FMCTAsyncCommandJob>();
	Job->TaskId = TaskId;
...
bool FMCTAsyncJobStore::MarkTaskRunning(const FString& TaskId)
{
	FScopeLock Lock(&CriticalSection);
	TSharedPtr<FMCTAsyncCommandJob>* FoundJob = Jobs.Find(TaskId);
...
void FMCTAsyncJobStore::CompleteTask(const FString& TaskId, const FString& ResponseJson, bool bSuccess, const FString& ErrorMessage)
{
	FScopeLock Lock(&CriticalSection);
	TSharedPtr<FMCTAsyncCommandJob>* FoundJob = Jobs.Find(TaskId);
	if (!FoundJob || !FoundJob->IsValid())
...
bool FMCTAsyncJobStore::RequestCancelTask(const FString& TaskId, FMCTAsyncCommandJob& OutJob)
{
	FScopeLock Lock(&CriticalSection);
	TSharedPtr<FMCTAsyncCommandJob>* FoundJob = Jobs.Find(TaskId);
	if (!FoundJob || !FoundJob->IsValid())
...
bool FMCTAsyncJobStore::TryCopyTask(const FString& TaskId, FMCTAsyncCommandJob& OutJob) const
{
	FScopeLock Lock(&CriticalSection);
	const TSharedPtr<FMCTAsyncCommandJob>* FoundJob = Jobs.Find(TaskId);
	if (!FoundJob || !FoundJob->IsValid())
	{
		return false;
	}
...
FMCTAsyncJobCounts FMCTAsyncJobStore::GetCounts() const
{
	FMCTAsyncJobCounts Counts;
	FScopeLock Lock(&CriticalSection);
	for (const TPair<FString, TSharedPtr<FMCTAsyncCommandJob>>& Pair : Jobs)
	{
...
TSharedPtr<FJsonObject> FMCTAsyncJobStore::BuildTaskJson(const FMCTAsyncCommandJob& Job, bool bIncludeResult) const
{
...
TSharedPtr<FJsonObject> FMCTAsyncJobStore::BuildTaskEventJson(const FMCTAsyncCommandEvent& Event) const
{
	TSharedPtr<FJsonObject> Data = MakeShared<FJsonObject>();
...
TSharedPtr<FJsonObject> FMCTAsyncJobStore::BuildTaskEventsJson(TSharedPtr<FJsonObject> Params) const
{
	FString TaskId;
	...
	int64 LatestSequence = 0;
	{
		...
		for (const FMCTAsyncCommandEvent& Event : Events)
		{
...
TSharedPtr<FJsonObject> FMCTAsyncJobStore::BuildTaskEventsWaitJson(TSharedPtr<FJsonObject> Params, TFunctionRef<bool()> IsStopRequested) const
{
...
FString FMCTAsyncJobStore::BuildTaskEventsSse(TSharedPtr<FJsonObject> Params) const
{
	TSharedPtr<FJsonObject> Data = BuildTaskEventsJson(Params);
	FString Output = TEXT(": MCPToolkit async task events\nretry: 1000\n\n");
...
TSharedPtr<FJsonObject> FMCTAsyncJobStore::BuildTaskEventParamsFromHttpRequest(const FHttpServerRequest& Request) const
{
...
TArray<TSharedPtr<FJsonValue>> FMCTAsyncJobStore::BuildTaskList(bool bIncludeFinishedOnly, bool bIncludeCancellableOnly, bool bIncludeResult) const
{
...
void FMCTAsyncJobStore::AppendTaskEventLocked(const FMCTAsyncCommandJob& Job, const FString& EventType, const FString& Message)
{
	FMCTAsyncCommandEvent Event;
