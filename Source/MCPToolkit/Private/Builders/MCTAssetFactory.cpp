...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MCTAssetFactory)
...
#define LOCTEXT_NAMESPACE "MCTAssetFactory"

DEFINE_LOG_CATEGORY_STATIC(LogMCTAssetFactory, Log, All);

// =============================================================================
// UTILITY
// =============================================================================

UClass* UMCTAssetFactory::ResolveAssetClass(const FString& AssetType)
{
	...
	UE_LOG(LogMCTAssetFactory, Warning, TEXT("ResolveAssetClass: Unsupported type '%s'"), *AssetType);
	return nullptr;
}

TArray<FString> UMCTAssetFactory::GetSupportedTypes()
{
	return {
		TEXT("InputAction"),
...
bool UMCTAssetFactory::SaveAsset(UObject* Asset)
{
	return UMCTDataAssetBuilder::SaveAsset(Asset);
}
...
UObject* UMCTAssetFactory::CreateAsset(
	...
	const TMap<FString, FString>& InitialProperties)
{
	...
	{
		UE_LOG(LogMCTAssetFactory, Error, TEXT("CreateAsset: PackagePath, AssetName and AssetType required"));
		return nullptr;
	}
	...
	if (!AssetClass)
	{
		UE_LOG(LogMCTAssetFactory, Error, TEXT("CreateAsset: Unknown asset type '%s'"), *AssetType);
		return nullptr;
	}
	...
	{
		UE_LOG(LogMCTAssetFactory, Warning, TEXT("CreateAsset: Already exists at '%s', returning existing"), *FullPath);
		return Existing;
	}
	...
	if (!Package)
	{
		UE_LOG(LogMCTAssetFactory, Error, TEXT("CreateAsset: Failed to create package at '%s'"), *FullPath);
		return nullptr;
	}
	...
	if (!NewAsset)
	{
		UE_LOG(LogMCTAssetFactory, Error, TEXT("CreateAsset: Failed to create %s '%s'"), *AssetType, *AssetName);
		return nullptr;
	}
	...
	{
		UMCTDataAssetBuilder::SetProperty(NewAsset, Pair.Key, Pair.Value);
	}

	FAssetRegistryModule::AssetCreated(NewAsset);
	NewAsset->MarkPackageDirty();

	UE_LOG(LogMCTAssetFactory, Log, TEXT("CreateAsset: Created %s '%s' at '%s'"),
...
static UClass* FindTriggerClass(const FString& ShortName)
{
	...
	UE_LOG(LogMCTAssetFactory, Warning, TEXT("FindTriggerClass: Not found '%s'"), *ShortName);
	return nullptr;
}
...
static UClass* FindModifierClass(const FString& ShortName)
{
	...
	UE_LOG(LogMCTAssetFactory, Warning, TEXT("FindModifierClass: Not found '%s'"), *ShortName);
	return nullptr;
}

bool UMCTAssetFactory::AddInputMapping(
	...
	const TArray<FString>& ModifierClasses)
{
	if (!IMC)
	{
		UE_LOG(LogMCTAssetFactory, Error, TEXT("AddInputMapping: Null IMC"));
		return false;
	}
	...
	if (!Action)
	{
		UE_LOG(LogMCTAssetFactory, Error, TEXT("AddInputMapping: InputAction not found at '%s'"), *InputActionPath);
		return false;
	}
	...
	if (!Key.IsValid())
	{
		UE_LOG(LogMCTAssetFactory, Error, TEXT("AddInputMapping: Invalid key name '%s'"), *KeyName);
		return false;
	}
	...
	UE_LOG(LogMCTAssetFactory, Log, TEXT("AddInputMapping: Added %s → %s to %s"),
		*Action->GetName(), *KeyName, *IMC->GetName());
	return true;
}

bool UMCTAssetFactory::RemoveInputMapping(UInputMappingContext* IMC, int32 MappingIndex)
{
	if (!IMC)
	{
		UE_LOG(LogMCTAssetFactory, Error, TEXT("RemoveInputMapping: Null IMC"));
		return false;
	}
	...
	{
		UE_LOG(LogMCTAssetFactory, Error, TEXT("RemoveInputMapping: Index %d out of range (0-%d)"),
	...
	UE_LOG(LogMCTAssetFactory, Log, TEXT("RemoveInputMapping: Removed mapping at index %d from %s"),
		MappingIndex, *IMC->GetName());
	return true;
}

TSharedPtr<FJsonObject> UMCTAssetFactory::GetInputMappingsAsJson(UInputMappingContext* IMC)
{
