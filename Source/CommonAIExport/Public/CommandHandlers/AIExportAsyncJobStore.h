...
{
struct FAIExportAsyncCommandJob
{
	FString TaskId;
	FString CommandName;
	FString Status;
...
struct FAIExportAsyncCommandEvent
{
	int64 Sequence = 0;
	FString TaskId;
	FString CommandName;
...
struct FAIExportAsyncJobCounts
{
	int32 QueuedTasks = 0;
...
public:
	...
	bool RequestCancelTask(const FString& TaskId, FAIExportAsyncCommandJob& OutJob);
	bool TryCopyTask(const FString& TaskId, FAIExportAsyncCommandJob& OutJob) const;

	FAIExportAsyncJobCounts GetCounts() const;
	TSharedPtr<FJsonObject> BuildTaskJson(const FAIExportAsyncCommandJob& Job, bool bIncludeResult) const;
...
private:
	TSharedPtr<FJsonObject> BuildTaskEventJson(const FAIExportAsyncCommandEvent& Event) const;
	void AppendTaskEventLocked(const FAIExportAsyncCommandJob& Job, const FString& EventType, const FString& Message);

	mutable FCriticalSection CriticalSection;
	TMap<FString, TSharedPtr<FAIExportAsyncCommandJob>> Jobs;
	TArray<FAIExportAsyncCommandEvent> Events;
