...
#include "AIExporterRegistry.generated.h"

class UAIExporterBase;
 ...
 *   if (UAIExporterBase* Exporter = Registry->FindExporterForAsset(MyAsset))
 *   {
 *       FString ExportedText = Exporter->Export(MyAsset, true);
 *   }
 */
UCLASS()
class COMMONAIEXPORT_API UAIExporterRegistry : public UObject
{
	GENERATED_BODY()
...
public:
	 ...
	 */
	void RegisterExporter(UAIExporterBase* Exporter);
	 ...
	 */
	const TArray<UAIExporterBase*>& GetRegisteredExporters() const { return RegisteredExporters; }
...
private:
	/** Singleton instance */
	static UAIExporterRegistry* Instance;

	/** Registered exporter instances, sorted by priority */
	UPROPERTY()
	TArray<TObjectPtr<UAIExporterBase>> RegisteredExporters;
