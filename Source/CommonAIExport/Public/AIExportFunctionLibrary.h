...
#include "AIExportFunctionLibrary.generated.h"
 ...
 */
DECLARE_DELEGATE_ThreeParams(FOnAIExportProgress, int32 /*Current*/, int32 /*Total*/, const FString& /*AssetName*/);

/**
 * Function library for AI Export operations.
 *
 * This is a FACADE that provides a clean public API for AI export functionality.
 * The actual export logic is delegated to the AIExporterRegistry and individual exporters.
 *
 ...
 */
UCLASS()
class COMMONAIEXPORT_API UAIExportFunctionLibrary : public UObject
{
	GENERATED_BODY()
...
public:
	 ...
	 */
	static int32 ExportAssets(const TArray<FAssetData>& Assets, FOnAIExportProgress OnProgress, TArray<FAIExportResult>& OutResults);
	...
	static FAIExportResult ExportAssetByPath(const FString& AssetPath, const FString& OutputDirectory, bool bBothFormats = true);

	/**
	 * Export a Widget Blueprint by path (convenience function)
	 */
	UFUNCTION(BlueprintCallable, Category = "AI Export")
	static FAIExportResult ExportWidgetBlueprintByPath(const FString& AssetPath, const FString& OutputDirectory, bool bBothFormats = true);

	/**
	 * Export a Blueprint by path (convenience function)
	 */
	UFUNCTION(BlueprintCallable, Category = "AI Export")
	static FAIExportResult ExportBlueprintByPath(const FString& AssetPath, const FString& OutputDirectory, bool bBothFormats = true);
