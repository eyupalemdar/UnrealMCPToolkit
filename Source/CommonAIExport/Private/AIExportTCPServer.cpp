...
TSharedPtr<FJsonObject> BuildRuntimeAudioComponentJson(UAudioComponent* AudioComponent)
{
	TSharedPtr<FJsonObject> Data = BuildRuntimeComponentJson(AudioComponent);
	if (!AudioComponent)
	{
		return Data;
	}

	Data->SetObjectField(TEXT("sound"), BuildRuntimeObjectReferenceJson(AudioComponent->Sound.Get()));
	Data->SetObjectField(TEXT("sound_class_override"), BuildRuntimeObjectReferenceJson(AudioComponent->SoundClassOverride.Get()));
	Data->SetObjectField(TEXT("attenuation_settings"), BuildRuntimeObjectReferenceJson(AudioComponent->AttenuationSettings.Get()));
	Data->SetObjectField(TEXT("source_effect_chain"), BuildRuntimeObjectReferenceJson(AudioComponent->SourceEffectChain.Get()));
...
{
	TSharedPtr<FJsonObject> Data = MakeShared<FJsonObject>();
	const bool bValid = Box.IsValid != 0;
...
{
	TSharedPtr<FJsonObject> Data = BuildRuntimeObjectReferenceJson(NavData);
	if (!NavData)
	{
		return Data;
	}
...
FString TruncateRuntimeDiagnosticString(const FString& Value, int32 Limit)
{
...
TSharedPtr<FJsonObject> BuildBlackboardKeyJson(UBlackboardComponent* Blackboard, UBlackboardData* Asset, FBlackboard::FKey KeyId, bool bIncludeValue, int32 StringLimit)
{
	...
	Data->SetObjectField(TEXT("type"), BuildRuntimeObjectReferenceJson(KeyType));
	...
	if (bIncludeValue)
	{
		Data->SetStringField(TEXT("value"), TruncateRuntimeDiagnosticString(Blackboard->DescribeKeyValue(KeyId, EBlackboardDescription::OnlyValue), StringLimit));
		Data->SetStringField(TEXT("description"), TruncateRuntimeDiagnosticString(Blackboard->DescribeKeyValue(KeyId, EBlackboardDescription::DetailedKeyWithValue), StringLimit));
	}
	return Data;
}

TSharedPtr<FJsonObject> BuildRuntimeBlackboardComponentJson(UBlackboardComponent* Blackboard, bool bIncludeValues, int32 KeyLimit, int32 StringLimit)
{
	TSharedPtr<FJsonObject> Data = BuildRuntimeComponentJson(Blackboard);
	...
	Data->SetObjectField(TEXT("asset"), BuildRuntimeObjectReferenceJson(Asset));
	...
	Data->SetObjectField(TEXT("parent_asset"), BuildRuntimeObjectReferenceJson(Asset->Parent));

	TArray<TSharedPtr<FJsonValue>> KeysJson;
	for (int32 KeyIndex = 0; KeyIndex < KeyCount; ++KeyIndex)
	{
		if (KeysJson.Num() >= KeyLimit)
		{
			break;
		}
	...
	if (bIncludeValues)
	{
		Data->SetStringField(TEXT("debug_info"), TruncateRuntimeDiagnosticString(Blackboard->GetDebugInfoString(EBlackboardDescription::Full), StringLimit));
	}
	return Data;
}

TSharedPtr<FJsonObject> BuildRuntimeBehaviorTreeComponentJson(UBehaviorTreeComponent* BehaviorTree, bool bIncludeDebugStrings, int32 StringLimit)
{
	TSharedPtr<FJsonObject> Data = BuildRuntimeComponentJson(BehaviorTree);
	...
	{
		Data->SetStringField(TEXT("debug_info"), TruncateRuntimeDiagnosticString(BehaviorTree->GetDebugInfoString(), StringLimit));
		Data->SetStringField(TEXT("active_tasks"), TruncateRuntimeDiagnosticString(BehaviorTree->DescribeActiveTasks(), StringLimit));
		Data->SetStringField(TEXT("active_trees"), TruncateRuntimeDiagnosticString(BehaviorTree->DescribeActiveTrees(), StringLimit));
	}
	return Data;
}

TSharedPtr<FJsonObject> BuildRuntimeBrainComponentJson(UBrainComponent* Brain, bool bIncludeDebugStrings, int32 StringLimit)
{
	TSharedPtr<FJsonObject> Data = BuildRuntimeComponentJson(Brain);
	...
	Data->SetObjectField(TEXT("ai_owner"), BuildRuntimeObjectReferenceJson(Brain->GetAIOwner()));
	Data->SetBoolField(TEXT("include_debug_strings"), bIncludeDebugStrings);
	if (bIncludeDebugStrings)
	{
		Data->SetStringField(TEXT("debug_info"), TruncateRuntimeDiagnosticString(Brain->GetDebugInfoString(), StringLimit));
	}

	if (UBehaviorTreeComponent* BehaviorTree = Cast<UBehaviorTreeComponent>(Brain))
	{
		Data->SetObjectField(TEXT("behavior_tree"), BuildRuntimeBehaviorTreeComponentJson(BehaviorTree, bIncludeDebugStrings, StringLimit));
	}
	else
	{
		Data->SetObjectField(TEXT("behavior_tree"), BuildRuntimeBehaviorTreeComponentJson(nullptr, bIncludeDebugStrings, StringLimit));
	}
	return Data;
}
...
TSharedPtr<FJsonObject> BuildNavigationPathJson(const FNavigationPath* Path, int32 PathPointLimit)
{
	...
	Data->SetObjectField(TEXT("base_actor"), BuildRuntimeObjectReferenceJson(Path->GetBaseActor()));
	Data->SetObjectField(TEXT("source_actor"), BuildRuntimeObjectReferenceJson(Path->GetSourceActor()));
	Data->SetObjectField(TEXT("querier"), BuildRuntimeObjectReferenceJson(Path->GetQuerier()));
	Data->SetObjectField(TEXT("navigation_data"), BuildRuntimeObjectReferenceJson(Path->GetNavigationDataUsed()));

	const TArray<FNavPathPoint>& PathPoints = Path->GetPathPoints();
	TArray<TSharedPtr<FJsonValue>> PointsJson;
	for (int32 PointIndex = 0; PointIndex < PathPoints.Num(); ++PointIndex)
	{
		if (PointsJson.Num() >= PathPointLimit)
		{
			break;
		}
...
TSharedPtr<FJsonObject> BuildRuntimePathFollowingComponentJson(UPathFollowingComponent* PathFollowing, bool bIncludePathPoints, int32 PathPointLimit, bool bIncludeDebugStrings, int32 StringLimit)
{
	TSharedPtr<FJsonObject> Data = BuildRuntimeComponentJson(PathFollowing);
	...
	Data->SetObjectField(TEXT("move_goal"), BuildRuntimeObjectReferenceJson(PathFollowing->GetMoveGoal()));
	Data->SetObjectField(TEXT("current_custom_link_object"), BuildRuntimeObjectReferenceJson(PathFollowing->GetCurrentCustomLinkOb()));
	Data->SetBoolField(TEXT("include_path_points"), bIncludePathPoints);
	Data->SetNumberField(TEXT("path_point_limit"), PathPointLimit);
	const FNavPathSharedPtr CurrentPath = PathFollowing->GetPath();
	Data->SetObjectField(TEXT("path"), bIncludePathPoints ? BuildNavigationPathJson(CurrentPath.Get(), PathPointLimit) : BuildRuntimeObjectReferenceJson(CurrentPath.IsValid() ? CurrentPath->GetNavigationDataUsed() : nullptr));
	if (bIncludeDebugStrings)
	{
		Data->SetStringField(TEXT("debug_string"), TruncateRuntimeDiagnosticString(PathFollowing->GetDebugString(), StringLimit));
	}
	return Data;
}

TSharedPtr<FJsonObject> BuildRuntimeAIControllerJson(
	AAIController* Controller,
	bool bIncludeBlackboardValues,
	bool bIncludeBehaviorDebug,
	bool bIncludePathPoints,
	bool bIncludePerception,
	int32 BlackboardKeyLimit,
	int32 PathPointLimit,
	int32 StringLimit)
{
	TSharedPtr<FJsonObject> Data = BuildRuntimeActorJson(Controller);
	...
	Data->SetObjectField(TEXT("focus_actor"), BuildRuntimeObjectReferenceJson(Controller->GetFocusActor()));
	Data->SetObjectField(TEXT("default_navigation_filter_class"), BuildRuntimeObjectReferenceJson(Controller->GetDefaultNavigationFilterClass().Get()));
	Data->SetObjectField(TEXT("pawn"), Pawn ? BuildRuntimeActorJson(Pawn) : BuildRuntimeObjectReferenceJson(nullptr));
	Data->SetObjectField(TEXT("player_state"), BuildRuntimeObjectReferenceJson(Controller->PlayerState));
	Data->SetObjectField(TEXT("brain"), BuildRuntimeBrainComponentJson(Controller->GetBrainComponent(), bIncludeBehaviorDebug, StringLimit));
	Data->SetObjectField(TEXT("blackboard"), BuildRuntimeBlackboardComponentJson(Controller->GetBlackboardComponent(), bIncludeBlackboardValues, BlackboardKeyLimit, StringLimit));
	Data->SetObjectField(TEXT("path_following"), BuildRuntimePathFollowingComponentJson(Controller->GetPathFollowingComponent(), bIncludePathPoints, PathPointLimit, bIncludeBehaviorDebug, StringLimit));
	...
	Data->SetObjectField(TEXT("perception"), bIncludePerception
		? BuildAIPerceptionComponentJson(Perception, TEXT(""), false, 0, 0)
		: BuildRuntimeComponentJson(Perception));
	return Data;
}
...
FString FAIExportTCPServer::HandleRuntimeAudioDiagnostics(TSharedPtr<FJsonObject> Params)
{
	...
	AsyncTask(ENamedThreads::GameThread, [WorldSelector, ActorPath, ActorLabel, ActorName, NameFilter, ClassFilter, ComponentClassFilter, SoundFilter, bIncludeInactive, ComponentLimit, Promise, this]()
	{
		TSharedPtr<FJsonObject> Data = MakeShared<FJsonObject>();
		Data->SetStringField(TEXT("requested_world"), WorldSelector);
		Data->SetObjectField(TEXT("pie"), BuildPIEStateJson());
		...
		for (AActor* Actor : ActorsToInspect)
		{
			...
			for (UAudioComponent* AudioComponent : AudioComponents)
			{
				...
				if (ComponentsJson.Num() < ComponentLimit)
				{
					ComponentsJson.Add(MakeShared<FJsonValueObject>(BuildRuntimeAudioComponentJson(AudioComponent)));
				}
			}
		}
...
FString FAIExportTCPServer::HandleRuntimeNavigationDiagnostics(TSharedPtr<FJsonObject> Params)
{
	...
	AsyncTask(ENamedThreads::GameThread, [WorldSelector, NavDataFilter, bIncludeNavData, bIncludeBounds, bIncludeSupportedAgents, bIncludeInvokers, NavDataLimit, BoundsLimit, InvokerLimit, TileSampleLimit, Promise, this]()
	{
		TSharedPtr<FJsonObject> Data = MakeShared<FJsonObject>();
		Data->SetStringField(TEXT("requested_world"), WorldSelector);
		Data->SetObjectField(TEXT("pie"), BuildPIEStateJson());
		...
		if (!NavSys)
		{
			...
			Data->SetObjectField(TEXT("navigation_system"), BuildRuntimeObjectReferenceJson(nullptr));
		...
		TSharedPtr<FJsonObject> NavSysJson = BuildRuntimeObjectReferenceJson(NavSys);
		...
		NavSysJson->SetObjectField(TEXT("main_nav_data"), BuildRuntimeObjectReferenceJson(NavSys->GetDefaultNavDataInstance()));
		NavSysJson->SetObjectField(TEXT("abstract_nav_data"), BuildRuntimeObjectReferenceJson(NavSys->GetAbstractNavData()));
...
FString FAIExportTCPServer::HandleRuntimeAIControllerDiagnostics(TSharedPtr<FJsonObject> Params)
{
	...
	AsyncTask(ENamedThreads::GameThread, [WorldSelector, ActorPath, ActorLabel, ActorName, NameFilter, ClassFilter, PawnFilter, bIncludeBlackboardValues, bIncludeBehaviorDebug, bIncludePathPoints, bIncludePerception, ControllerLimit, BlackboardKeyLimit, PathPointLimit, DebugStringLimit, Promise, this]()
	{
		TSharedPtr<FJsonObject> Data = MakeShared<FJsonObject>();
		Data->SetStringField(TEXT("requested_world"), WorldSelector);
		Data->SetObjectField(TEXT("pie"), BuildPIEStateJson());
		...
		for (TActorIterator<AAIController> It(World); It; ++It)
		{
			...
			if (UBrainComponent* Brain = Controller->GetBrainComponent())
			{
				...
				if (Cast<UBehaviorTreeComponent>(Brain))
				{
					++BehaviorTreeComponentCount;
				}
			}
			if (Controller->GetBlackboardComponent())
			{
				++BlackboardComponentCount;
			}
			if (Controller->GetPathFollowingComponent())
			{
				++PathFollowingComponentCount;
			}
			if (Controller->GetMoveStatus() == EPathFollowingStatus::Moving)
			{
				++MovingControllerCount;
			}
			if (Controller->GetAIPerceptionComponent())
			{
				++PerceptionComponentCount;
			}
		}
		...
		for (AAIController* Controller : ControllersToInspect)
		{
			...
			ControllersJson.Add(MakeShared<FJsonValueObject>(BuildRuntimeAIControllerJson(
				Controller,
