...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AIMaterialBuilder)
...
UMaterial* UAIMaterialBuilder::CreateMaterial(
	...
	bool bTwoSided)
{
	...
	{
		UE_LOG(LogAIMaterialBuilder, Error, TEXT("CreateMaterial: PackagePath and AssetName required"));
		return nullptr;
	}
	...
	{
		UE_LOG(LogAIMaterialBuilder, Warning, TEXT("CreateMaterial: Already exists at '%s', returning existing"), *FullPath);
		return Existing;
	}
	...
	if (!Package)
	{
		UE_LOG(LogAIMaterialBuilder, Error, TEXT("CreateMaterial: Failed to create package at '%s'"), *FullPath);
		return nullptr;
	}
	...
	if (!NewMaterial)
	{
		UE_LOG(LogAIMaterialBuilder, Error, TEXT("CreateMaterial: Factory failed for '%s'"), *AssetName);
		return nullptr;
	}
	...
	UE_LOG(LogAIMaterialBuilder, Log, TEXT("CreateMaterial: Created '%s' (Domain=%s, Blend=%s, Shading=%s)"),
		*FullPath, *Domain, *BlendMode, *ShadingModel);
	return NewMaterial;
}

bool UAIMaterialBuilder::SetMaterialProperty(
	UMaterial* Material,
	...
	const FString& Value)
{
	...
	{
		UE_LOG(LogAIMaterialBuilder, Warning, TEXT("SetMaterialProperty: Failed '%s'='%s' on '%s'"),
			*PropertyName, *Value, *Material->GetName());
	}
	return bSuccess;
}

bool UAIMaterialBuilder::CompileMaterial(
	UMaterial* Material,
	TArray<FString>* OutWarnings)
{
	if (!Material)
	{
		UE_LOG(LogAIMaterialBuilder, Error, TEXT("CompileMaterial: Material is null"));
		return false;
	}
	...
	UE_LOG(LogAIMaterialBuilder, Log, TEXT("CompileMaterial: '%s' compiled, saved=%s"),
		*Material->GetName(), bSaved ? TEXT("true") : TEXT("false"));
	return true;
}

UMaterial* UAIMaterialBuilder::LoadMaterial(const FString& AssetPath)
{
	...
	if (!Material)
	{
		UE_LOG(LogAIMaterialBuilder, Error, TEXT("LoadMaterial: Not found at '%s'"), *AssetPath);
	}
	return Material;
}
...
UMaterialExpression* UAIMaterialBuilder::AddExpression(
	UMaterial* Material,
	...
	int32 PosY)
{
	if (!Material)
	{
		UE_LOG(LogAIMaterialBuilder, Error, TEXT("AddExpression: Material is null"));
		return nullptr;
	}
	...
	if (!ExprClass)
	{
		UE_LOG(LogAIMaterialBuilder, Error, TEXT("AddExpression: Cannot resolve class '%s'"), *ExpressionClass);
		return nullptr;
	}
	...
	if (!Expr)
	{
		UE_LOG(LogAIMaterialBuilder, Error, TEXT("AddExpression: CreateMaterialExpression failed for '%s'"),
			*ExpressionClass);
		return nullptr;
	}
	...
	UE_LOG(LogAIMaterialBuilder, Log, TEXT("AddExpression: Added '%s' (%s) at (%d,%d)"),
		*NodeName, *ExpressionClass, PosX, PosY);

	return Expr;
}

bool UAIMaterialBuilder::SetExpressionProperty(
	UMaterial* Material,
	const FString& NodeName,
	const FString& PropertyName,
	const FString& Value)
{
	...
	if (!Expr)
	{
		UE_LOG(LogAIMaterialBuilder, Error, TEXT("SetExpressionProperty: Node '%s' not found"), *NodeName);
		return false;
	}
	...
	{
		UE_LOG(LogAIMaterialBuilder, Warning, TEXT("SetExpressionProperty: Failed '%s'='%s' on node '%s'"),
			*PropertyName, *Value, *NodeName);
	}
	return bSuccess;
}

bool UAIMaterialBuilder::ConnectExpressions(
	UMaterial* Material,
	...
	const FString& ToInput)
{
	...
	if (!FromExpr)
	{
		UE_LOG(LogAIMaterialBuilder, Error, TEXT("ConnectExpressions: From node '%s' not found"), *FromNodeName);
		return false;
	}
	...
	if (!ToExpr)
	{
		UE_LOG(LogAIMaterialBuilder, Error, TEXT("ConnectExpressions: To node '%s' not found"), *ToNodeName);
		return false;
	}
	...
	Material->MarkPackageDirty();
	UE_LOG(LogAIMaterialBuilder, Log, TEXT("ConnectExpressions: '%s'.%s -> '%s'.%s"),
		*FromNodeName, *FromOutput, *ToNodeName, *ToInput);

	return true;
}

bool UAIMaterialBuilder::ConnectToMaterialProperty(
	UMaterial* Material,
	const FString& FromNodeName,
	const FString& FromOutput,
	const FString& MaterialProperty)
{
	...
	if (!FromExpr)
	{
		UE_LOG(LogAIMaterialBuilder, Error, TEXT("ConnectToMaterialProperty: Node '%s' not found"), *FromNodeName);
		return false;
	}
	...
	if (Prop == MP_MAX)
	{
		UE_LOG(LogAIMaterialBuilder, Error, TEXT("ConnectToMaterialProperty: Unknown property '%s'"), *MaterialProperty);
		return false;
	}
	...
	Material->MarkPackageDirty();
	UE_LOG(LogAIMaterialBuilder, Log, TEXT("ConnectToMaterialProperty: '%s'.%s -> %s (result=%s)"),
		*FromNodeName, *FromOutput, *MaterialProperty, bResult ? TEXT("true") : TEXT("false"));

	return bResult;
}

bool UAIMaterialBuilder::DisconnectInput(
	UMaterial* Material,
	const FString& NodeName,
	const FString& InputName)
{
	...
	UE_LOG(LogAIMaterialBuilder, Warning, TEXT("DisconnectInput: Input '%s' not found on node '%s'"),
		*InputName, *NodeName);
	return false;
}

bool UAIMaterialBuilder::RemoveExpression(
	UMaterial* Material,
	const FString& NodeName)
{
	if (!Material) return false;
	...
	UE_LOG(LogAIMaterialBuilder, Log, TEXT("RemoveExpression: Removed '%s'"), *NodeName);
	return true;
}
...
UMaterialInstanceConstant* UAIMaterialBuilder::CreateMaterialInstance(
	const FString& PackagePath,
	const FString& AssetName,
	const FString& ParentMaterialPath)
{
	...
	{
		UE_LOG(LogAIMaterialBuilder, Warning, TEXT("CreateMaterialInstance: Already exists at '%s'"), *FullPath);
		return Existing;
	}
	...
	if (!ParentMaterial)
	{
		UE_LOG(LogAIMaterialBuilder, Error, TEXT("CreateMaterialInstance: Parent material not found: '%s'"), *ParentMaterialPath);
		return nullptr;
	}
	...
	if (!MIC)
	{
		UE_LOG(LogAIMaterialBuilder, Error, TEXT("CreateMaterialInstance: Factory failed"));
		return nullptr;
	}
	...
	UE_LOG(LogAIMaterialBuilder, Log, TEXT("CreateMaterialInstance: Created '%s' with parent '%s'"),
		*FullPath, *ParentMaterialPath);
	return MIC;
}

bool UAIMaterialBuilder::SetInstanceParameter(
	...
	const FString& Value)
{
	...
	else if (ParamType.Equals(TEXT("vector"), ESearchCase::IgnoreCase))
	{
		...
		UE_LOG(LogAIMaterialBuilder, Error, TEXT("SetInstanceParameter: Failed to parse vector value '%s'"), *Value);
		return false;
	}
	else if (ParamType.Equals(TEXT("texture"), ESearchCase::IgnoreCase))
	{
		...
		if (!Texture)
		{
			UE_LOG(LogAIMaterialBuilder, Error, TEXT("SetInstanceParameter: Texture not found: '%s'"), *Value);
			return false;
		}
	...
	UE_LOG(LogAIMaterialBuilder, Error, TEXT("SetInstanceParameter: Unknown param_type '%s'. Use 'scalar', 'vector', or 'texture'"), *ParamType);
	return false;
}

bool UAIMaterialBuilder::SaveMaterialInstance(UMaterialInstanceConstant* MIC)
{
	if (!MIC) return false;
...
UMaterialInstanceConstant* UAIMaterialBuilder::LoadMaterialInstance(const FString& AssetPath)
{
	FString CleanPath = AssetPath;
	...
	if (!MIC)
	{
		UE_LOG(LogAIMaterialBuilder, Error, TEXT("LoadMaterialInstance: Not found at '%s'"), *AssetPath);
	}
	return MIC;
}

TSharedPtr<FJsonObject> UAIMaterialBuilder::GetMaterialInstanceInfoAsJson(UMaterialInstanceConstant* MIC)
{
...
UMaterialExpression* UAIMaterialBuilder::FindExpressionByName(
	UMaterial* Material,
	const FString& NodeName)
{
...
UClass* UAIMaterialBuilder::ResolveExpressionClass(const FString& ClassNameOrPath)
{
	// Full path support
	...
	UE_LOG(LogAIMaterialBuilder, Warning, TEXT("ResolveExpressionClass: Cannot resolve '%s'"), *ClassNameOrPath);
	return nullptr;
}

TMap<FString, UClass*>& UAIMaterialBuilder::GetExpressionClassMap()
{
...
TArray<FString> UAIMaterialBuilder::GetAvailableExpressionClasses()
{
...
EMaterialProperty UAIMaterialBuilder::ResolveMaterialProperty(const FString& PropertyName)
{
...
TSharedPtr<FJsonObject> UAIMaterialBuilder::ExpressionToJson(UMaterialExpression* Expr, int32 Index)
{
...
TSharedPtr<FJsonObject> UAIMaterialBuilder::GetMaterialGraphAsJson(UMaterial* Material)
{
	TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();
	if (!Material)
	{
...
bool UAIMaterialBuilder::SetPropertyByPath(
	UObject* Object,
	...
	const FString& Value)
{
	...
	for (int32 i = 0; i < PathParts.Num(); ++i)
	{
		...
		if (!Prop)
		{
			UE_LOG(LogAIMaterialBuilder, Warning, TEXT("SetPropertyByPath: Property '%s' not found on %s"),
			...
			if (Enum)
			{
				...
				for (int32 EnumIdx = 0; EnumIdx < Enum->NumEnums() - 1; ++EnumIdx)
				{
						...
						DisplayName.Equals(CleanValue, ESearchCase::IgnoreCase))
					{
						...
						UE_LOG(LogAIMaterialBuilder, Verbose,
				...
				UE_LOG(LogAIMaterialBuilder, Warning,
...
bool UAIMaterialBuilder::SaveAsset(UObject* Asset)
{
