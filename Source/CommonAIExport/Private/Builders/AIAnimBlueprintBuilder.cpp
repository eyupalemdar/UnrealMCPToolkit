...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AIAnimBlueprintBuilder)
...
#define LOCTEXT_NAMESPACE "AIAnimBlueprintBuilder"

DEFINE_LOG_CATEGORY_STATIC(LogAIAnimBlueprintBuilder, Log, All);

// =============================================================================
// UTILITY
// =============================================================================

UAnimBlueprint* UAIAnimBlueprintBuilder::LoadAnimBlueprint(const FString& AssetPath)
{
	...
	if (!AnimBP)
	{
		UE_LOG(LogAIAnimBlueprintBuilder, Error, TEXT("LoadAnimBlueprint: Not found at '%s'"), *AssetPath);
	}
	return AnimBP;
}
...
static UClass* ResolveAnimParentClass(const FString& ParentClass)
{
	...
	UE_LOG(LogAIAnimBlueprintBuilder, Warning,
		TEXT("ResolveAnimParentClass: '%s' not found, using UAnimInstance"), *ParentClass);
	return UAnimInstance::StaticClass();
}

UAnimBlueprint* UAIAnimBlueprintBuilder::CreateAnimBlueprint(
	...
	const FString& ParentClass)
{
	...
	{
		UE_LOG(LogAIAnimBlueprintBuilder, Error,
	...
	UE_LOG(LogAIAnimBlueprintBuilder, Log,
		TEXT("CreateAnimBlueprint: Created '%s' (Skeleton=%s, Parent=%s)"),
		*FullPath, *Skeleton->GetName(), *AnimParentClass->GetName());
	return NewAnimBP;
}

bool UAIAnimBlueprintBuilder::CompileAndSave(UAnimBlueprint* AnimBP, TArray<FString>* OutWarnings)
{
	if (!AnimBP)
	{
		UE_LOG(LogAIAnimBlueprintBuilder, Error, TEXT("CompileAndSave: Null AnimBlueprint"));
		return false;
	}
	...
	{
		bHasErrors = true;
		UE_LOG(LogAIAnimBlueprintBuilder, Error, TEXT("CompileAndSave: Compilation failed for '%s'"),
			*AnimBP->GetName());
	}
	...
	if (!bHasErrors)
	{
		...
		UE_LOG(LogAIAnimBlueprintBuilder, Log, TEXT("CompileAndSave: %s '%s' (Saved=%d)"),
...
TSharedPtr<FJsonObject> UAIAnimBlueprintBuilder::GetAnimBlueprintInfoAsJson(UAnimBlueprint* AnimBP)
{
