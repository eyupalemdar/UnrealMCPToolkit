...
bool UAITextureExporter::CanExport(UObject* Asset) const
{
	if (!Asset)
	{
		return false;
...
TArray<UClass*> UAITextureExporter::GetSupportedClasses() const
{
	return {
...
FString UAITextureExporter::Export(UObject* Asset, bool bFilterDefaults)
{
...
FString UAITextureExporter::ExportTextureMetadata(UTexture2D* Texture, bool bFilterDefaults)
{
	FString Output;
...
bool UAITextureExporter::ExportTextureToPNG(UTexture2D* Texture, const FString& OutputPath)
{
	if (!Texture)
	{
		UE_LOG(LogTemp, Error, TEXT("AITextureExporter: Texture is null"));
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Error, TEXT("AITextureExporter: No platform data or mips available for %s"), *Texture->GetName());
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Error, TEXT("AITextureExporter: Failed to create ImageWrapper for PNG"));
		return false;
	}
	...
	if (PixelFormat == PF_B8G8R8A8 || PixelFormat == PF_R8G8B8A8 || PixelFormat == PF_G8)
	{
		...
		if (!RawData)
		{
			UE_LOG(LogTemp, Error, TEXT("AITextureExporter: Failed to lock texture data for %s"), *Texture->GetName());
			return false;
		}
		...
		if (Texture->Source.IsValid())
		{
			...
			if (Texture->Source.GetMipData(SourceData, 0))
			{
				...
				default:
					...
					UE_LOG(LogTemp, Log, TEXT("AITextureExporter: Source format %d for %s, using BGRA conversion"),
			...
			{
				UE_LOG(LogTemp, Error, TEXT("AITextureExporter: Failed to get source mip data for %s"), *Texture->GetName());
				return false;
			}
		}
		else
		{
			UE_LOG(LogTemp, Error, TEXT("AITextureExporter: Compressed texture %s (format %d) has no source data available"),
	...
	if (!bSetRawSuccess)
	{
		UE_LOG(LogTemp, Error, TEXT("AITextureExporter: Failed to set raw image data for %s"), *Texture->GetName());
		return false;
	}
	...
	if (PNGData.Num() == 0)
	{
		UE_LOG(LogTemp, Error, TEXT("AITextureExporter: Failed to compress image to PNG for %s"), *Texture->GetName());
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Error, TEXT("AITextureExporter: Failed to save PNG to %s"), *OutputPath);
		return false;
	}

	UE_LOG(LogTemp, Log, TEXT("AITextureExporter: Successfully exported %s to %s (%dx%d)"),
