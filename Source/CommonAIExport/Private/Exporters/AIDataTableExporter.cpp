...
bool UAIDataTableExporter::CanExport(UObject* Asset) const
{
	return Asset && Asset->IsA<UDataTable>();
}

TArray<UClass*> UAIDataTableExporter::GetSupportedClasses() const
{
	return { UDataTable::StaticClass() };
}

FString UAIDataTableExporter::Export(UObject* Asset, bool bFilterDefaults)
{
...
FString UAIDataTableExporter::ExportDataTable(UDataTable* DataTable, bool bFilterDefaults)
{
	FString Output;
...
FString UAIDataTableExporter::ExportRow(UScriptStruct* RowStruct, const FName& RowName, const uint8* RowData) const
{
	FString Output;
