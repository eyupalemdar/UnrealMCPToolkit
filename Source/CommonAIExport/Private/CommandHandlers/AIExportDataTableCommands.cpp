...
FString HandleCreateDataTable(TSharedPtr<FJsonObject> Params)
{
	...
	{
		FString Error;
		TSharedPtr<FJsonObject> Data = UAIDataTableBuilder::CreateDataTable(PackagePath, AssetName, RowStructPath, Error);
		if (!Data.IsValid())
		{
...
FString HandleGetDataTableInfo(TSharedPtr<FJsonObject> Params)
{
	...
	{
		FString Error;
		UDataTable* DataTable = UAIDataTableBuilder::LoadDataTable(AssetPath, Error);
		if (!DataTable)
		{
			return CreateBuilderErrorResponse(Error, TEXT("Failed to load DataTable"));
		}
		return CreateSuccessResponse(UAIDataTableBuilder::BuildDataTableInfoJson(DataTable));
...
FString HandleReadDataTableRows(TSharedPtr<FJsonObject> Params)
{
	...
	{
		FString Error;
		TSharedPtr<FJsonObject> Data = UAIDataTableBuilder::ReadRows(AssetPath, RowNameFilter, Limit, Error);
		if (!Data.IsValid())
		{
...
FString HandleAddDataTableRow(TSharedPtr<FJsonObject> Params)
{
	...
	{
		FString Error;
		TSharedPtr<FJsonObject> Data = UAIDataTableBuilder::AddRow(AssetPath, RowName, Values, bSave, Error);
		if (!Data.IsValid())
		{
...
FString HandleRemoveDataTableRow(TSharedPtr<FJsonObject> Params)
{
	...
	{
		FString Error;
		TSharedPtr<FJsonObject> Data = UAIDataTableBuilder::RemoveRow(AssetPath, RowName, bSave, Error);
		if (!Data.IsValid())
		{
...
FString HandleImportDataTableCsv(TSharedPtr<FJsonObject> Params)
{
	...
	{
		FString Error;
		TSharedPtr<FJsonObject> Data = UAIDataTableBuilder::ImportCsv(AssetPath, CsvText, CsvFilePath, bSave, Error);
		if (!Error.IsEmpty())
		{
