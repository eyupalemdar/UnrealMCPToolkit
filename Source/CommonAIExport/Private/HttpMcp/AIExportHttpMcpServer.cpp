...
FAIExportHttpMcpStatus FAIExportHttpMcpServer::GetStatus() const
{
	FAIExportHttpMcpStatus Status;
...
const TArray<FString>* FAIExportHttpMcpServer::FindHttpHeaderValues(const FHttpServerRequest& Request, const FString& HeaderName) const
{
...
FString FAIExportHttpMcpServer::GetHttpHeaderValue(const FHttpServerRequest& Request, const FString& HeaderName) const
{
...
bool FAIExportHttpMcpServer::IsHttpOriginAllowed(const FString& Origin) const
{
	if (Origin.IsEmpty())
	{
		return true;
...
bool FAIExportHttpMcpServer::IsHttpRequestAllowed(const FHttpServerRequest& Request) const
{
...
bool FAIExportHttpMcpServer::IsMcpProtocolVersionAllowed(const FHttpServerRequest& Request, FString& OutError) const
{
...
void FAIExportHttpMcpServer::PruneExpiredMcpSessionsLocked(const FDateTime& NowUtc)
{
...
bool FAIExportHttpMcpServer::DeleteMcpSession(const FString& SessionId)
{
	FScopeLock Lock(&ActiveMcpSessionsCriticalSection);
...
void FAIExportHttpMcpServer::AppendHttpAuditEvent(const FHttpServerRequest& Request, const FString& Route, int32 ResponseCode, bool bAllowed, const FString& Detail, const FString& McpSessionId) const
{
...
TUniquePtr<FHttpServerResponse> FAIExportHttpMcpServer::MakeHttpJsonResponse(const FString& Json, int32 ResponseCode, const FString& McpSessionId) const
{
	return MakeHttpTextResponse(Json, TEXT("application/json"), ResponseCode, McpSessionId);
}

TUniquePtr<FHttpServerResponse> FAIExportHttpMcpServer::MakeHttpTextResponse(const FString& Text, const FString& ContentType, int32 ResponseCode, const FString& McpSessionId) const
{
...
void FAIExportHttpMcpServer::Start(const FAIExportHttpMcpCallbacks& InCallbacks)
{
...
void FAIExportHttpMcpServer::Stop()
{
	if (HttpRouter.IsValid())
	{
	...
	Callbacks = FAIExportHttpMcpCallbacks();
	{
		FScopeLock Lock(&ActiveMcpSessionsCriticalSection);
...
FString FAIExportHttpMcpServer::HandleMcpJsonRpc(const FString& RequestJson, const FString& RequestSessionId, FString& OutSessionId)
{
	...
	if (Method == TEXT("tools/list"))
	{
		...
		const TArray<FAIExportHttpMcpToolDescriptor> Descriptors = Callbacks.GetToolDescriptors ? Callbacks.GetToolDescriptors() : TArray<FAIExportHttpMcpToolDescriptor>();
		const int32 EndIndex = FMath::Min(StartIndex + PageSize, Descriptors.Num());

		TArray<TSharedPtr<FJsonValue>> Tools;
		for (int32 Index = StartIndex; Index < EndIndex; ++Index)
		{
			const FAIExportHttpMcpToolDescriptor& Descriptor = Descriptors[Index];
			...
			Tool->SetStringField(TEXT("description"), FString::Printf(TEXT("CommonAIExport %s command. category=%s scope=%s dry_run=%s"), *Descriptor.Name, *Descriptor.Category, *Descriptor.RequiredScope, Descriptor.bSupportsDryRun ? TEXT("true") : TEXT("false")));
