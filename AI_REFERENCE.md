...
> 133 MCP tools across 24 TCP command categories plus client-only tools. UE 5.7, TCP port auto-discovery plus multi-editor routing and native localhost HTTP/MCP probe support.
...
*133 MCP tools, covering Widget, Material, BP Graph, CDO, Asset, Import, Preview, editor/level/actor/PIE/runtime inspector, AI Perception, CommonUI runtime routing, asset streaming, logs/workflow source-control status/history/diff, resources/prompts, client scope policy, native HTTP/MCP probe, registry metadata, multi-editor routing, code transfer, async job, and contract introspection workflows*
