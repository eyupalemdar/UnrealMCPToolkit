    ...
    class Visitor(ast.NodeVisitor):
        def visit_Call(self, node: ast.Call) -> None:  # noqa: N802 - ast visitor API
            ...
            if isinstance(node.func, ast.Name) and node.func.id == "_send_command":
                ...
                command_arg_index = 1
            elif isinstance(node.func, ast.Name) and node.func.id == "_send_generated_tcp_tool":
                command_arg_index = 0
...
def build_wrapper_runtime(command_manifest: dict, wrapper_spec: dict) -> str:
    ...
    for name in sorted(command_by_name):
        ...
        specs[name] = {
            ...
            "meta_params": runtime_meta_params,
        }
    ...
    lines = [
        ...
        "from __future__ import annotations",
        "",
        "GENERATED_TCP_WRAPPERS = " + pprint.pformat(specs, width=120, sort_dicts=True),
...
def build_server_metadata(command_manifest: dict, tool_schemas: dict) -> dict:
    return {
        ...
        "tools": {
            ...
            "generated_runtime_imported": True,
        },
...
def write_artifacts(output_dir: Path = GENERATED_DIR) -> dict[str, Path]:
    ...
    wrapper_runtime = build_wrapper_runtime(command_manifest, wrapper_spec)
    ...
    targets["wrapper_runtime"].write_text(wrapper_runtime, encoding="utf-8")
